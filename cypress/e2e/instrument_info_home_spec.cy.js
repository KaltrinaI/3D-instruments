describe('Instrument Details Page', () => {
  beforeEach(() => {
    cy.visit('http://127.0.0.1:8000/catalogue/3/latest/72495c80-1df3-11ef-910f-05ae3f26d1aa');
  });

  it('should load the instrument details page', () => {
    cy.get('.title h3').should('contain.text', 'Yamaha FG800');
  ...
  it('should display warning message if present', () => {
    cy.get('body').then(($body) => {
  ...
  it('should display the correct instrument description', () => {
    cy.get('.text-monospace').first().should('not.be.empty'); });

    it('should display the correct instrument family', () => {
      cy.get('h5').contains('Instrument family').parent().within(() => {
        cy.get('span.text-muted').eq(0).should('have.text', 'Strings'); // Use eq(0) to get the first span
      });
    });
    

  it('should display the correct instrument price', () => {
    cy.get('h5').contains('Price').parent().find('span.text-muted').should('contain.text', '$');
  });

  it('should display the correct instrument stock', () => {
    cy.get('h5').contains('In stock').parent().within(() => {
      cy.get('span.text-muted').eq(2).should('have.text', '8');
    });
  });
